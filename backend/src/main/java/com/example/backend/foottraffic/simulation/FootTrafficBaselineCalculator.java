...
public final class FootTrafficBaselineCalculator {
    ...
    public static BaselineResult calc(
            PoiTagCounts counts,
            FootTrafficZoneParams params,
            double popDensity,
            double avgIncome,
            String h3Index,
            long globalSalt) {
        double poiCap = Math.max(1.0, params.getPoiDensityCap());
        double popCap = Math.max(1.0, params.getPopDensityCap());
        double incW = Math.max(0.0, params.getIncomeWeight());
