...
export default function ProfileInput() {
  ...
  const submit = async () => {
    ...
    setError(null);
    try {
      const p = await generateDynamicProfile(query.trim());
      setProfile(p);
      setProfileId(p.id);
    } catch {
      setError("Échec de la génération — vérifiez la connexion ou réessayez.");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div>
      ...
      <button
        ...
        style={{
          ...
          marginTop: "8px",
          padding: "9px",
          background: loading || !query.trim() ? "#94a3b8" : "#1a56db",
          color: "#fff",
          border: "none",
          borderRadius: "10px",
          fontSize: "12.5px",
...
function ProfileCard({ profile, onReset }: { profile: DynamicProfileResponse; onReset: () => void }) {
  return (
    <div style={{ background: "#f8fafc", borderRadius: "12px", padding: "12px", border: "1px solid #e2e8f0" }}>
      ...
      <p style={{ fontSize: "11px", color: "#64748b", marginBottom: "10px", lineHeight: 1.4 }}>
        {profile.userQuery.slice(0, 80)}
        {profile.userQuery.length > 80 ? "…" : ""}
      </p>
